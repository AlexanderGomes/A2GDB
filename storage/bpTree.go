...
func (t *Tree) Find(key uint64, verbose bool) (*Record, error) {
	i := 0
...
func (t *Tree) FindAndPrint(key uint64, verbose bool) {
...
func (t *Tree) FindAndPrintRange(key_start, key_end uint64, verbose bool) {
	var i int
	array_size := key_end - key_start + 1
	returned_keys := make([]uint64, array_size)
...
func (t *Tree) findRange(key_start, key_end uint64, verbose bool, returned_keys []uint64, returned_pointers []interface{}) int {
	var i int
	num_found := 0
	...
	// Skip keys before key_start
	for i = 0; i < n.NumKeys && n.Keys[i] < key_start; i++ {
	}

	for n != nil {
		for ; i < n.NumKeys && n.Keys[i] <= key_end; i++ {
			if num_found >= len(returned_keys) {
				returned_keys = append(returned_keys, 0)
				returned_pointers = append(returned_pointers, nil)
			}
...
func (t *Tree) findLeaf(key uint64, verbose bool) *Node {
	i := 0
	c := t.Root
	if c == nil {
		if verbose {
...
func insertIntoLeaf(leaf *Node, key uint64, pointer *Record) {
...
func (t *Tree) insertIntoLeafAfterSplitting(leaf *Node, key uint64, pointer *Record) error {
	var new_leaf *Node
...
func insertIntoNode(n *Node, left_index int, key uint64, right *Node) {
	var i int
...
func (t *Tree) insertIntoNodeAfterSplitting(old_node *Node, left_index int, key uint64, right *Node) error {
	var i, j, split int
	var k_prime uint64
	var new_node, child *Node
	var temp_keys []uint64
...
func (t *Tree) insertIntoParent(left *Node, key uint64, right *Node) error {
	var left_index int
	parent := left.Parent
...
func (t *Tree) insertIntoNewRoot(left *Node, key uint64, right *Node) error {
	t.Root, err = makeNode()
	if err != nil {
		return err
	}
...
func (t *Tree) startNewTree(key uint64, pointer *Record) error {
	t.Root, err = makeLeaf()
	if err != nil {
		return err
	}
...
func removeEntryFromNode(n *Node, key uint64, pointer interface{}) *Node {
...
func (t *Tree) coalesceNodes(n, neighbour *Node, neighbour_index int, k_prime uint64) {
...
func (t *Tree) redistributeNodes(n, neighbour *Node, neighbour_index, k_prime_index int, k_prime uint64) {
	var i int
	var tmp *Node
...
func (t *Tree) deleteEntry(n *Node, key uint64, pointer interface{}) {
